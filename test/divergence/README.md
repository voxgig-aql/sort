...
```bash
boru X            # interpreter — the default; what CI and users run   (GATING)
boru check X      # static type-check                                  (ADVISORY)
...
interpreter run, or any difference between `boru --compile X` and `boru X`,
fails the script. That interpreter == byte-compiler equality is the real
correctness guarantee.

`boru check` is **reported but not gating**. Its static analysis has known
...
runs `boru check --soft` on the library as advisory — see
...
`run.sh` builds its own boru at a ref pinned in the script (the same `12a44e0`
...
`boru --compile X` and `boru X`; `check` columns are informational. Needs `go`
...
by `boru-lang/boru`'s `design/COMPILABLE-SUBSET.md` ("refusal is always sound;
